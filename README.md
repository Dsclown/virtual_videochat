...
| Avatar | Playwright Headless + Cubism Web SDK 渲染 Live2D |
...
浏览器                          服务端
  ...
  │      (JSON 控制面)            │
  │                               │
  │◄════ WebRTC (SRTP) ═════════►│  AvatarVideoTrack  ← Playwright 帧
...
| 层级 | 现状 | 可替换为 |
|------|------|----------|
| **媒体源** | TTS → PCM 队列；Playwright → RGB 帧 | 不变，或改为 canvas 流 / 客户端渲染 |
...
├── frontend/                 # Web 前端（WebRTC + canvas 显示）
├── assets/live2d/            # Cubism Core + 模型（见 assets/live2d/README.md）
├── render-engine/
│   └── cubism-sdk-live2d/    # Live2D 渲染页源码（Vite 构建 → avatar.js）
...
│       ├── modules/
│       │   ├── avatar/       # Playwright 渲染、WebRTC tracks、口型
│       │   ├── tts/          # TtsSession 按序下发
...
```yaml
avatar:
  fps: 10                      # 抓帧目标（受 CPU 限制，Playwright 截图为瓶颈）
