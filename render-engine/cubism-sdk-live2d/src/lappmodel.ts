...
export class LAppModel extends CubismUserModel {
  ...
  public update(): void {
    ...
    this._applyExternalMouth();

    this._model.update();
  }

  private _applyExternalMouth(): void {
    if (this._model == null) {
      return;
    }
    const v = Math.max(0, Math.min(1, this._externalMouth));
    if (this._lipSyncIds.length > 0) {
      for (let i = 0; i < this._lipSyncIds.length; i++) {
        this._model.setParameterValueById(this._lipSyncIds[i], v);
      }
      return;
    }
    const mouthId = CubismFramework.getIdManager().getId('PARAM_MOUTH_OPEN_Y');
    if (mouthId != null) {
      this._model.setParameterValueById(mouthId, v);
    }
  }
  ...
  public setMouthOpenY(value: number): void {
    ...
    this._externalMouth = Math.max(0, Math.min(1, Number(value) || 0));
  }
}
